"""Exercise the actual curve menu and Blob Worker, with the built CSP in force.
Set VS_TEST_URL for source/Pages. Default uses isolated single HTML (not an
IndexedDB persistence test). Fixtures are generated, never customer images.
"""
...
with sync_playwright() as p:
    ...
    ready();base=snapshot();poly=paths()
    ...
    page.locator('#trace-curve-controls').scroll_into_view_if_needed()
    page.screenshot(path=str(OUT/'bezier-controls-v031.png'))
    ...
    assert snapshot()['shapes'] and snapshot()!=generated
    assert not errors,errors
    print('PASS: visible curve controls, disabled state, real Q/C worker geometry, latest request, slider, protected corners, unchanged preview document, Apply parity, undo/redo, SVG, manual generation; CSP and version checked.')
    browser.close()
