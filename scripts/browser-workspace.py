...
with sync_playwright() as p:
 ...
 page=context.new_page();page.set_default_timeout(15000);errors=[];page.on('pageerror',lambda e:errors.append(str(e)));page.on('dialog',lambda d:d.accept())
 ...
 if INJECT:
  ...
  save();id_a=page.evaluate('vectorStudio.activeProject()');page.click('#projects-btn');expect(project('项目 A')).to_have_count(1);page.screenshot(path=str(OUT/'projects-v020.png'));page.click('#close-projects')
  ...
  page.click('#projects-btn');expect(project('项目 B')).to_have_count(1);expect(project('不应保存的 B')).to_have_count(0);page.click('#close-projects')
